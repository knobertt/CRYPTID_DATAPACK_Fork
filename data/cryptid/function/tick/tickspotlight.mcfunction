...
##extend
execute as @e[type=vindicator, tag=cryptid.spotlight2] if score @s cryptid.mob.id = @n[type=vindicator, tag=cryptid.spotlight,distance=..1] cryptid.mob.id run tag @s add this
...
execute at @s anchored eyes run tp @e[tag=cryptid.spotlight2, type=vindicator, sort=nearest, limit=1, tag=this] ^ ^ ^60
execute as @e[type=vindicator, tag=cryptid.spotlight2,tag = this] run tag @s remove this
...
execute if score @s cryptid.player.random matches 1 run execute at @s as @e[type=vindicator, tag=cryptid.spotlight2] if score @s cryptid.mob.id = @n[type=vindicator, tag=cryptid.spotlight,distance=..1] cryptid.mob.id run function cryptid:events/quietkill
